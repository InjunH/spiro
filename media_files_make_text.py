...
def authenticate_vito(CLIENT_ID, CLIENT_SECRET):
    try:
        ...
        logger.info("인증 성공! Access Token: " + access_token)
        return access_token
...
def call_vito_api_with_token(file_path, access_token):
    ...
    try:
        ...
        if 'id' in response_data:
            ...
            logger.info(f"파일 {file_path} 처리 완료. TRANSCRIBE_ID: {transcribe_id}")
            return transcribe_id
        else:
            logger.error(f"파일 {file_path} 처리 중 알 수 없는 응답: {response_data}")
            return None
    except requests.HTTPError as e:
        handle_http_error(e)
        sys.exit(1)
...
def get_transcribe_result(transcribe_id, file_name, access_token, today, retry_count=0):
    try:
        ...
        if response_data.get('status') == 'completed':
            ...
            logger.info(f"결과가 {json_file_path}에 저장되었습니다.")
            ...
            logger.info(f"텍스트 결과가 {txt_file_path}에 저장되었습니다.")
